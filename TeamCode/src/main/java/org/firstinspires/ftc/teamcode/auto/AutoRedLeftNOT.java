...
@Config
@Autonomous(name = "NOT Auto Red Left")
public class AutoRedLeftNOT extends CommandOpMode
{
    ...
    boolean resetElevator = false, openElevator = false, releaseIntake = false, extendIntake = false, canTransfer = true, releasePreload = false;
    ...
    TrajectorySequence placePurplePixel, intakeWithPreload, placePreloadAndIntake, placePreload, goIntake, park, placeCycle;
    ...
    public void initialize() {
        ...
        placePurplePixel = drivetrain.trajectorySequenceBuilder(autoConstants.startPose)
                .addTemporalMarker(1, () -> intake.setAngle(Intake.Angle.INTAKE))
                .forward(AutoConstants.strafeForPurplePixel)
                .addDisplacementMarker(() -> intake.updateClawState(Intake.ClawState.OPEN, ClawSide.LEFT))
                .build();

        intakeWithPreload = drivetrain.trajectorySequenceBuilder(placePurplePixel.end())
                .turn(Math.toRadians(-90))
                .build();

        placePreloadAndIntake = drivetrain.trajectorySequenceBuilder(placePreload.end())
        ...
        goIntake = drivetrain.trajectorySequenceBuilder(placePreloadAndIntake.end())
        ...
        placeCycle = drivetrain.trajectorySequenceBuilder(goIntake.end())
                .lineToSplineHeading(autoConstants.stageDoorMidPose)
                .lineToSplineHeading(autoConstants.stageDoorEndPose)
                .splineToLinearHeading(autoConstants.placePixelPose, Math.toRadians(0))
                .build();

        park = drivetrain.trajectorySequenceBuilder(placeCycle.end())
        ...
        switch (robotState)
        {
            ...
            case WAIT_PLACE_PURPLE_PIXEL:
                if (waitTimer1.seconds() >= waitTillPlacePurple) {
                    ...
                    drivetrain.followTrajectorySequenceAsync(intakeWithPreload);
                }
                break;
            case INTAKE_1_PIXEL:
                if (!drivetrain.isBusy()) {
            ...
            case WAIT_INTAKE_CYCLE_1:
                if (waitTimer4.seconds() >= waitTillOuttakePixel) {
                    ...
                    drivetrain.followTrajectorySequenceAsync(placeCycle);
                }
                break;
    ...
    void intakeStateMachine()
    {
        switch (intakeState) {
            ...
            case INTAKE:
                ...
                if (robot.has2Pixels() && !startedDelayTransfer && canTransfer) {
                    had2Pixels = true;
